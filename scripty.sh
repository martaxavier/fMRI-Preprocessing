...
for i in "${subj_list[@]}"; do  
  ...
  #flirt -in GM_thr -ref example_func -applyxfm -init unwarp/highres2example_func.mat -interp nearestneighbour -out EF_GM_thr
...
#  fslmaths $path/$dataset/DERIVATIVES/$i/bet_fast/${i}_T1_crop_restore_brain_pve_1.nii* -thr 1 GM_thr.nii.gz
  ...
  cp $path/$dataset/DATA/$i/$run/func/$func_data_raw_sub epi.nii.gz
